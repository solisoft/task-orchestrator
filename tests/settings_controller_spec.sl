...
describe("SettingsController", fn()
  ...
  describe("codex agent", fn()
    ...
    test("renders codex in the agent type dropdown", fn()
      let response = get("/settings")
      let body = res_body(response)
      assert_contains(body, "value=\"codex\"")
    end)

    test("renders enable checkbox for codex", fn()
      let response = get("/settings")
      let body = res_body(response)
      assert_contains(body, "id=\"enabled_codex\"")
      assert_contains(body, "name=\"enabled_codex\"")
    end)

    test("renders daily and weekly limit inputs for codex", fn()
      let response = get("/settings")
      let body = res_body(response)
      assert_contains(body, "name=\"limit_daily_codex\"")
      assert_contains(body, "name=\"limit_weekly_codex\"")
    end)
